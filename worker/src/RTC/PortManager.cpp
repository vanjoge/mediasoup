...
namespace RTC
{
    ...
    uv_handle_t* PortManager::Bind(Transport transport, std::string& ip, int useReserved)
    {
        MS_TRACE();
        ...
        if (useReserved==0)
        {
            ...
            portIdx = static_cast<size_t>(Utils::Crypto::GetRandomUInt(
              static_cast<uint32_t>(100), static_cast<uint32_t>(ports.size() - 1)));
        }
        else if (useReserved == 1)
        {
            //1 从0开始
            portIdx = 0;
        }
