...
namespace RTC
{
    ...
    static std::unordered_map<std::string, RTC::SingleUdpSocket*> singleUdpSocket;
        ...
        try
        {
            ...
            for (auto& listenIp : listenIps)
            {
                if (enableUdp)
                {
...
#ifdef SINGLE_UDP_PORT
                    // This may throw.
                    RTC::SingleUdpSocket* udpSocket = NULL;
                    ...
                    {
                        udpSocket = new RTC::SingleUdpSocket(listenIp.ip);
    ...
    void WebRtcTransport::HandleRequest(Channel::Request* request)
    {
        ...
        switch (request->methodId)
        {
            ...
            case Channel::Request::MethodId::TRANSPORT_RESTART_ICE:
            {
                ...
                for (auto& item : singleUdpSocket)
                {
                    item.second->ChangeTransportByUserName(
                      this, iceServer->GetUsernameFragment(), usernameFragment);
                }
